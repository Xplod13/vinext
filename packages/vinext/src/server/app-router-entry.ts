...
// Virtual module generated by vinext at build time. The default export is the
// request handler; `__assetPrefix` and `__basePath` are named exports inlined
// from next.config (both as string literals) so this worker can recognise
// asset-shaped requests without touching the handler. Types live in
// `virtual-vinext-rsc-entry.d.ts`; the generator is in `entries/app-rsc-entry.ts`.
import rscHandler, {
...
async function handleRequest(
  ...
  // Invalid `_next/static/*` paths short-circuit with a plain-text 404 instead
  // of falling through to the RSC handler (which would render the full HTML
  // 404 page). Valid assets are served by Cloudflare's ASSETS binding BEFORE
  // the worker is invoked; only misses reach this code. Matches Next.js:
  // packages/next/src/server/lib/router-server.ts:661-668.
